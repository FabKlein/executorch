...
Tensor& avg_pool2d_f32_out(
  ...
  (void)stride;
  (void)padding;
  fail_cmsis_float_kernel_unavailable(context, "avg_pool2d_f32_out", "float32");
  return out;
#else
  ...
  const arm_cmsis_nn_status status = arm_avg_pool_f32(
      &cmsis_ctx,
      &pool_params,
      &input_dims,
  ...
  if (status != ARM_CMSIS_NN_SUCCESS) {
    ET_LOG(
        Error,
        "avg_pool2d_f32_out: arm_avg_pool_f32 failed with status [%d]",
...
Tensor& avg_pool2d_f16_out(
  ...
  (void)stride;
  (void)padding;
  fail_cmsis_float_kernel_unavailable(context, "avg_pool2d_f16_out", "float16");
  return out;
#else
  static_assert(
  ...
  fill_float_pool_params<float16_t>(
      pool_params, pad_h, pad_w, stride_h, stride_w);
  cmsis_nn_context cmsis_ctx{nullptr, 0};
  const arm_cmsis_nn_status status = arm_avg_pool_f16(
      &cmsis_ctx,
      &pool_params,
      &input_dims,
  ...
  if (status != ARM_CMSIS_NN_SUCCESS) {
    ET_LOG(
        Error,
        "avg_pool2d_f16_out: arm_avg_pool_f16 failed with status [%d]",
        static_cast<int>(status));
    context.fail(Error::Internal);
  }
  return out;
